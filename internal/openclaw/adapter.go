...
type Adapter struct {
	...
	mu  sync.Mutex
	db  *sql.DB
	err error // sticky: the first open error is remembered for /healthz
}
...
func (a *Adapter) openLocked(ctx context.Context) error {
	...
	if err != nil {
		a.err = fmt.Errorf("openclaw: open database: %w", err)
		return a.err
	}
	...
	if !schema.Compatible() {
		_ = db.Close()
		a.err = &SchemaError{Missing: schema.MissingRequired}
		return a.err
	}
...
func (a *Adapter) Ping(ctx context.Context) error {
	...
	if err := a.db.QueryRowContext(ctx, "SELECT 1").Scan(&n); err != nil {
		a.err = fmt.Errorf("openclaw: ping: %w", err)
		return a.err
	}
	if n != 1 {
